...
//
var _app = function (elm, elm_target) {
    return elm.appendChild(elm_target);
};
...
var _css = function (e, hash) {
    ...
    for (var k in hash) {
        ...
        if (map[k]) {
            ...
            // margin-top を marginTop に置換するような処理
            k_map = k.replace(
                /\-(\w)/g,
                function(c) {
...
var _addClass = function (e, classname) {
    var classlist =
        e.className.replace(/(^\s*|\s*$)/g, '').split(/\s+/);
...
Mario.prototype = {
    ...
    __init: function (param) {
        ...
        if (typeof Mario.__BGDATA[this.__NAME][this.__SCALE] == 'undefined') {
            this.__NAME = 'MARIO';
        }
        ...
        this.__BODY = _elm(
            ...
            },
            {
                ...
                background:
                    'url('
                        + Mario.__BGDATA[this.__NAME][this.__SCALE]
                        + ') left top no-repeat'
            }
        );
    ...
    __switch_bg: function () {
        var key   = [this.STATUS, this.ACTION].join('_')
          , index = Mario.__BGINFO[key].INDEX
                        + (this.__is_direction_left()  ?  1  :  0)
        ;
    ...
    __switch_animation: function () {
        ...
        this.__TIMER_ANIMATION = setInterval(
            function () {
    ...
    update_velocity: function (v, flg_bdash) {
        ...
        case v < 0:
            this.LEFT();
            if (this.__VELOCITY_X == 0) {
                this.__VELOCITY_X = - this.__VELOCITY_ZERO_RANGE;
            } // 初速
            this.__VELOCITY_X -=
                this.__ABILITY.ACCEL * (flg_bdash  ?  this.__ABILITY.BDASH  :  1);
    ...
    __daemon: function () {
        var __this = this;
        this.__TIMER_DAEMON = setInterval(
            function() {
...
//                }
    ...
    WALK: function (param) {
        ...
        var __this = this;
        this.__TIMER_WALK = setInterval(
            function () {
                __this.update_velocity(
                    __this.__is_direction_right()  ?  1  :  -1,
                    bdash
                );
                // distance 指定時，その距離を経過したら静止する
                // duration 指定時，その時間が経過したら静止する
                if(
                    (distance > 0  &&  Math.abs( x0 - __this.__X ) >= distance)  ||
...
Mario.Controller.prototype = {
    __init: function (param) {
        ...
        window.onkeydown = function (ev) {
            __this.__keydown(ev);
            if (typeof __window_onkeydown == 'function') {
                __window_onkeydown(ev);
            }
        };
        window.onkeyup = function (ev) {
            __this.__keyup(ev);
            if (typeof __window_onkeyup == 'function') {
                __window_onkeyup( ev );
            }
        };
