...
//
var _elm = function( name, attr, css ) {
  var elm = document.createElement( name );
  if( typeof attr != 'undefined' ) { _attr( elm, attr ); }
  if( typeof css != 'undefined' ) { _css( elm, css ); }
  return elm;
};
//
var _app = function( elm, elm_target ) { return elm.appendChild( elm_target ); };

/**
 *
 *  要素 e に hash で指定したスタイルの組を追加する
 *
 *  @param  HTMLDOMElement(?)  対象となる要素
 *  @param  Object             追加するスタイルのハッシュ
 *
 **/
var _css = function( e, hash ) {
  // css での名称と javascript における style プロパティでの名称が異なる場合，ここに記述する
  var map = {
    float:  'cssFloat'
  };
  for( var k in hash ) {
    var v = hash[ k ];
    var k_map = k;
    if( map[ k ] ) { k_map = map[ k ]; }
    else           { k_map = k.replace( /\-(\w)/g, function( c ) { return c.toUpperCase().replace( /\-/g, '' ); } ); }  //  margin-top を marginTop に置換するような処理
...
/**
 *
 *  要素 e に hash で指定した属性の組を追加する
 *  
 *  @param  HTMLDOMElement(?)  対象となる要素
 *  @param  Object             追加する属性のハッシュ
 *
 **/
var _attr = function( e, hash ) {
  // HTMLにおける属性名と，それに対応する javascript での名称が異なる場合，ここに記述する
  var map = {
    'class': 'className'
  };
  for( var k in hash ) {
    ...
    e[ k_map ] = v;
    //e.setAttribute( k_map, v );  // 動かない？
  }
};

/**
 *
 *  要素 e のクラス名に classname を追加する
 *
 *  @param  String  クラス名
 *
 **/
var _addClass = function( e, classname ) {
  var classlist = e.className.replace( /(^\s*|\s*$)/g, '' ).split( /\s+/ );
  ...
  if( ! class_exists ) { classlist.push( classname ); }
  _attr( e, { 'class': classlist.join( ' ' ) } );
};
 ...
 *  要素 e の幅/高さを取得する
 *
 *  @param   HTMLDOMObject(?)  取得対象の要素
 *
 *  @return  Integer  幅/高さ [px]
 *
 **/
var _width = function( e ) {
  return e.offsetWidth;
};
var _height = function( e ) {
  return e.offsetHeight;
};


/**
 *  要素 e のドキュメント上の位置を取得する
 *
 *  @param   HTMLDOMObject(?)  取得対象の要素
 *
 *  @return  Object
 ...
 **/
var _pos = function( e ) {
  return { x: _pos_x( e ), y: _pos_y( e ) };
};
var _pos_x = function( e ) {
  var x = 0, e_ = e;
  while( e_.offsetLet ) { x += e_.offsetLeft; e_ = e_.offsetParent; }
  return x;
};
var _pos_y = function( e ) {
  var y = 0, e_ = e;
  while( e_.offsetTop ) { y += e_.offsetTop; e_ = e_.offsetParent; }
  return y;
};
...
//
var create_class = function() { return function() { this.__init.apply( this, arguments ); }; };
 ...
 *
 **--------------------------------------------------------------------------------*/
var Mario = create_class();
Mario.KEYCODE_LEFT  = 37;
...
Mario.prototype = {
  __init: function( param ) {
    ...
    this.__WIDTH  = 16 * this.__SCALE;
    this.__HEIGHT = 32 * this.__SCALE;


    this.__VELOCITY_ZERO_RANGE = .4;
    this.__VELOCITY_X_MAX = 5;
    this.__VELOCITY_X = 0;
    this.__VELOCITY_Y = 0;

    this.__ABILITY = {
      ACCEL: ability.accel  ||  .25,
      BDASH: ability.bdash  ||  2,
      JUMP:  ability.jump   ||  6,
    };

    if( typeof Mario.__BGDATA[ this.__NAME ][ this.__SCALE ] == 'undefined' ) { this.__NAME = 'MARIO'; }
    ...
    this.__TIMER_WALK;
    this.__INTERVAL_WALK = 40;
    this.__T_WALK = 0;

    this.__TIMER_JUMP;

    this.__flg_jump   = false;
    this.__flg_break  = false;
    this.__flg_crouch = false;

    // マリオの体: div要素
    this.__BODY = _elm(
      'div',
      {
        id:        'mario-made-by-javascript',
      ...
      },
      {
        ...
        background: 'url(' + Mario.__BGDATA[ this.__NAME ][ this.__SCALE ] + ') left top no-repeat'
      }
    );

    this.__set_environment( environment );

    // 向き: String: 'RIGHT' or 'LEFT'
    this.DIRECTION = 'RIGHT';
    // 状態: String: 'MINI', 'SUPER', 'FIRE'
    this.STATUS = param.status  ||  'MINI';
    // Bダッシュ状態: Boolean
    this.B_DASH = false;
    ...
    //this.__set_position( this.__X, this.__Y );
    this.__start();
  },

  //
  __set_environment: function( env ) {
    this.ENVIRONMENT = env;
    return true;
  },

  //
  __set_status: function( s ) {
    this.STATUS = s;
    this.__switch_bg();
    return true;
  },
  //
  __set_action: function( a ) {
  ...
  //
  __is_status_mini:  function() { return this.STATUS == 'MINI';  },
  __is_status_super: function() { return this.STATUS == 'SUPER'; },
  __is_status_fire:  function() { return this.STATUS == 'FIRE';  },

  //
  __is_action_stand:  function() { return this.ACTION == 'STAND';  },
  __is_action_walk:   function() { return this.ACTION == 'WALK';   },
  __is_action_jump:   function() { return this.ACTION == 'JUMP';   },
  __is_action_crouch: function() { return this.ACTION == 'CROUCH'; },
  __is_action_swim1:  function() { return this.ACTION == 'SWIM1';  },
  __is_action_swim2:  function() { return this.ACTION == 'SWIM2';  },
  __is_action_die:    function() { return this.ACTION == 'DIE';    },

  //
  __is_direction_right: function() { return this.DIRECTION == 'RIGHT'; },
  __is_direction_left:  function() { return this.DIRECTION == 'LEFT';  },
   ...
   *
   **/
  __set_position: function( x, y ) {
    _css( this.__BODY, {
      top:  parseInt( y - this.__HEIGHT ),
      left: parseInt( x )
    } );
  },
  /**
   *
   **/
  __update_position: function() { this.__set_position( this.__X, this.__Y ); },



  __switch_bg: function() {
    var key   = [ this.STATUS, this.ACTION ].join( '_' )
      , index = Mario.__BGINFO[ key ].INDEX + ( this.__is_direction_left()  ?  1  :  0 )
    ;
    this.__set_bg_position( key, index, 0 );
    this.__switch_animation();
  },

  __set_bg_position: function( key, index, frame ) {
    this.__BGPOSITION_LEFT = - this.__WIDTH * index;
    this.__BGPOSITION_TOP  = - this.__HEIGHT * frame;
    _css( this.__BODY, {
      backgroundPosition: [ this.__BGPOSITION_LEFT + 'px', this.__BGPOSITION_TOP + 'px' ].join( ' ' )
    } );

  },

  __switch_animation: function() {
    var __this = this;
    var key    = [ this.STATUS, this.ACTION ].join( '_' )
      , index  = Mario.__BGINFO[ key ].INDEX + ( this.__is_direction_left()  ?  1  :  0 )
      , frames = Mario.__BGINFO[ key ].FRAMES
      //, frame  = 0
    ;

    clearInterval( this.__TIMER_ANIMATION );
    this.__TIMER_ANIMATION = setInterval( function() {
      __this.__FRAME_ANIMATION = ++__this.__FRAME_ANIMATION % frames;
      __this.__set_bg_position( key, index, __this.__FRAME_ANIMATION );
      //frame = ++frame % frames;
      //__this.__set_bg_position( key, index, frame );
    }, this.__INTERVAL_ANIMATION );
  },


  __update_interval: function() {
  ...
  update_velocity: function( v, flg_bdash ) {
    ...
    case v < 0:
      this.LEFT();
      if( this.__VELOCITY_X == 0 ) { this.__VELOCITY_X = - this.__VELOCITY_ZERO_RANGE; } // 初速
      this.__VELOCITY_X -= this.__ABILITY.ACCEL * ( flg_bdash  ?  this.__ABILITY.BDASH  :  1 );
      if( this.__VELOCITY_X > this.__VELOCITY_ZERO_RANGE ) {
        this.BREAK();
      }
      else {
        //this.WALK();
        if( ! this.__flg_jump ) {
          this.__set_action( 'WALK' );
        }
      }
  ...
  __start: function() {
    this.__switch_bg();
    this.__daemon();
    _app( document.body, this.__BODY );
  },

  __daemon: function() {
    var __this = this;
    this.__TIMER_DAEMON = setInterval( function() {
      var v_x = __this.__VELOCITY_X
      ;
      // ジャンプしていない場合は自然減速する
//      if( ! __this.__flg_jump ) {
        v_x *= .89; // 減速する．この値は要調整
//      }
      __this.__X += v_x;

      // 現在の速度が，速度0引き込み圏内にある
      if( Math.abs( v_x ) <= __this.__VELOCITY_ZERO_RANGE ) {
        v_x = 0;
        if( __this.__flg_break ) {
          __this.__flg_break = false;
        }
        
        if( __this.__VELOCITY_Y == 0 ) {
          if( ! __this.__flg_crouch ) { __this.STAND(); }
          //__this.__flg_crouch  ?  __this.CROUCH()  :  __this.STAND();
          //if( __this.__flg_jump  &&  __this.__flg_crouch ) __this.STAND();
        }
        else {
      ...
      __this.__set_position( __this.__X, __this.__Y );
    }, this.__INTERVAL_DAEMON );
   ...
   **/
  RIGHT: function() {
    if( this.__flg_jump ) { return false; }
    if( ! this.__flg_break  &&  this.__is_direction_right() ) { return false; }
    this.DIRECTION = 'RIGHT';
    this.__switch_bg();
  },
  /**
  ...
  LEFT: function() {
    if( this.__flg_jump ) { return false; }
    if( ! this.__flg_break  &&  this.__is_direction_left() ) { return false; }
    this.DIRECTION = 'LEFT';
    this.__switch_bg();
  },
  ...
  STAND: function() {
    this.__flg_crouch = false;
    //this.__set_action( this.__flg_jump ? 'JUMP' : 'STAND' );
    this.__set_action( this.__flg_jump ? 'JUMP' : 'STAND' );
  },
   ...
   **/
  WALK: function( param ) {
    if( this.__VELOCITY_X ) { return false; }
    this.STAND();
    this.__T_WALK = 0;
    ...
    var __this = this;
    this.__TIMER_WALK = setInterval( function() {
      __this.update_velocity(  __this.__is_direction_right()  ?  1  :  -1, bdash );
      // distance 指定時，その距離を経過したら静止する
      // duration 指定時，その時間が経過したら静止する
      if(  ( distance > 0  &&  Math.abs( x0 - __this.__X ) >= distance )
           ||  ( duration > 0  &&   ++__this.__T_WALK * __this.__INTERVAL_WALK >= duration )  ) {
        __this.STOP();
      }
    }, this.__INTERVAL_WALK );
...
*/
   ...
   **/
  STOP: function() {
    clearInterval( this.__TIMER_WALK );
  },
  ...
  BREAK: function() {
    if( this.__flg_jump ) { return false; }
    this.__flg_break = true;
    this.__set_action( 'BREAK' );
    return true;
  },
   ...
   **/
  JUMP: function() {
    if( this.__flg_jump ) { return; }
    this.__flg_jump = true;
    if( ! this.__flg_crouch ) {
      this.__set_action( 'JUMP' );
    }
    var Y_JUMP_START = this.__Y;

    var __this = this;
    var t = 0;
    this.__TIMER_JUMP = setInterval( function() {
      t += .05;
      __this.__VELOCITY_Y = __this.__ABILITY.JUMP - __this.ENVIRONMENT.GRAVITY * t * t;
      __this.__Y -= __this.__VELOCITY_Y;
      __this.__update_position();

      if( __this.__Y >= Y_JUMP_START ) {
        clearInterval( __this.__TIMER_JUMP );
        __this.__VELOCITY_Y = 0;
        __this.__Y = Y_JUMP_START;
        __this.__update_position();
        __this.__flg_jump = false;
      }
    }, 40 );
  },
  ...
  CROUCH: function() {
    clearInterval( this.__TIMER_WALK );
    if( this.__is_status_mini() ) { return; }
    if( this.__flg_jump )         { return; }
    this.__flg_crouch = true;
    this.__set_action( 'CROUCH' );
  },
   ...
   **/
  SWIM: function() {
    this.__set_action( 'SWIM' );
  },

  DIE: function( param ) {
    param = param  ||  {};
    var callback = param.callback;
    this.__set_action( 'DIE' );
 ...
 *
 *
 *  Mario.Environmentクラス
 *
 *    マリオが存在する環境を定義する（泳げるとか泳げないとかを）  
 *
 *
 *
 **--------------------------------------------------------------------------------*/
Mario.Environment = create_class();
Mario.Environment.prototype = {
  /**
   *  コンストラクタ
   *
   *  @param  ?gravity
  ...
  __init: function( param ) {
    if( typeof param == 'undefined' ) { param = {}; }
    this.GRAVITY   = typeof param.gravity != 'undefined'  ?  param.gravity  :  9.8;
    this.SWIMMABLE = param.swimmable  ||  false;
  },

  swimmable: function() { return this.SWIMMABLE; }
};
 ...
 *  Mario.Controllerクラス
 *
 *    キーボードでマリオ操作するための処理を定義する
 *
 *
 *
 *
 **--------------------------------------------------------------------------------*/
Mario.Controller = create_class();
Mario.Controller.prototype = {
  __init: function( param ) {
    ...
    var __window_onkeydown = window.onkeydown
      , __window_onkeyup   = window.onkeyup
    ;
    window.onkeydown = function( ev ) {
      __this.__keydown( ev );
      if( typeof __window_onkeydown == 'function' ) { __window_onkeydown( ev ); }
    };
    window.onkeyup = function( ev ) {
      __this.__keyup( ev );
      if( typeof __window_onkeyup == 'function' ) { __window_onkeyup( ev ); }
    };
